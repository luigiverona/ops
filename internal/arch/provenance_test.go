...
func (r *configRunner) Run(_ context.Context, s run.Spec) (run.Result, error) {
	...
	case "stat":
		...
		if strings.HasSuffix(s.Args[len(s.Args)-1], "/pacman.conf") || strings.HasSuffix(s.Args[len(s.Args)-1], ".db") || strings.HasSuffix(s.Args[len(s.Args)-1], ".pkg.tar") || strings.Contains(s.Args[len(s.Args)-1], "/var/cache/pacman/pkg/") {
	...
	case "sha256sum":
		path := s.Args[len(s.Args)-1]
		return run.Result{Stdout: fmt.Sprintf("%x  %s\n", sha256.Sum256(r.files[path]), path)}, nil
	case "install":
		if s.Stdin == nil && (s.Args[2] == "-d" || strings.Contains(strings.Join(s.Args, " "), " -T ")) {
		...
		r.files[s.Args[len(s.Args)-1]] = data
	...
	for _, operation := range []string{"-S", "-U"} {
		for _, failure := range []string{"", "copy", "stage", "pacman"} {
			...
			for _, call := range r.calls {
				...
				if call.Args[1] != "pacman" && call.Args[1] != "unshare" {
					continue
				}
				mutated = true
				if call.Interactive != spec.Interactive || call.Stdin != nil || !strings.Contains(strings.Join(call.Args, " "), "--config /var/tmp/ops-paru-CONFIG123456/pacman.conf -- extra/git") || strings.Contains(r.staged, "custom") || (operation == "-S" && !strings.Contains(r.staged, "[multilib]")) || (operation == "-U" && strings.Contains(r.staged, "[core]")) || !strings.Contains(r.staged, "PackageRequired") {
...
func TestInstallQualifiesTargetsWithoutPromotingImplicitDependencies(t *testing.T) {
	...
	for _, s := range r.calls {
		if s.Name == "sudo" && len(s.Args) > 1 && s.Args[1] == "unshare" {
			if strings.Join(testpkg.TransactionSpec(s).Args, " ") != "-n pacman -S --noconfirm -- extra/firefox" {
